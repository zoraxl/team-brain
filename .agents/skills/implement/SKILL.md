---
name: implement
description: Implement one or more reviewed phase plans in the target implementation repo. For wip plans, runs an explicit review-and-lock loop before implementation: confirm the user has reviewed the plan, resolve or route every open question, flip to ready to ship, then treat the locked plan as implementation scope. Runs /simplify as a post-implementation code-quality pass and tells the user they can proceed to /evaluate. Use when the user says "/implement", "implement this plan", "build this phase", "start phase implementation", or asks to turn a phase plan into code.
---

# Implement

Implement a written phase plan in the target repo while preserving the plan lifecycle. This skill bridges `/planning` and `/evaluate`; it writes code, runs a focused code-quality pass with `/simplify` when available, and leaves final acceptance verification to `/evaluate`.
...
- Do not implement a plan that still has unresolved open questions or has not been explicitly locked for implementation.
- Do not mark plans `implemented-pending-pr`; `/evaluate` owns that status transition after verifying acceptance criteria.
...
## Step 2 - Review and Lock the Phase

Handle each phase by status:

- `ready to ship` - treat the phase as already locked and proceed to implementation.
- `wip` - enter an interactive review-and-lock loop before implementation:
  - Ask the user to confirm the phase has been reviewed and should be locked for implementation.
  - If the user does not confirm, stop and leave the phase at `status: wip`.
  - If open questions remain, ask the user to bucket each as resolved now, empirical test, or backlog.
  ...
  - Only after the user confirms readiness and every open question is resolved or routed, remove the empty Open Questions section and change `status: wip` to `status: ready to ship`.
...
- Any phase files flipped to `ready to ship`.
- `/simplify` result and checks run.
- Any known gaps, deferred cleanup, or tests not run.
- A clear handoff: tell the user they can proceed to `/evaluate`.

Use this exact final handoff sentence when implementation appears complete:

> Implementation is complete; you can proceed to `/evaluate`.
