...
2. File lists and headings for `inbox/**/*.md`, `plans/**/*.md`, `wiki/logs/*.md`, and `archive/**/*.md` if present.
...
5. For confirmed legacy backfill before `/wiki-sync`, write enough metadata for the normal verification path:
   - Source idea: `namespace`, `status`, `related_plan`, `related_pr`, `wiki_log` when known.
   - Plan phases: `namespace`, `status`, `source_dump`, `related_pr`, `wiki_log` when known.
   - Preserve `archive_after`, `archived_from`, and `archived_at` if already present.
   - Use `status: planned` for active source ideas, `status: implemented-pending-pr` for implemented phases without a PR stamp, `status: pr-open` only when a real PR URL is known, and `status: implemented-and-synced` only when the PR is already in `wiki/logs/synced-prs.md`.
