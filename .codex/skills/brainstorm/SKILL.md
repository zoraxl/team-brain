...
Saved brainstorm files are source idea files in the lifecycle chain:

```text
brainstorm -> planned -> implemented-pending-pr -> pr-open -> implemented-and-synced -> archived
brainstorm -> resolved -> archived (for durable no-implementation outputs)
```

- `/brainstorm` owns only the initial `status: brainstorm` state.
- `/planning` later changes the source idea to `status: planned` and sets `related_plan` to a phase file or feature folder.
- `/implement` marks linked plan phases `implemented-pending-pr` after implementation and `/simplify`; `/evaluate` may also do so when the user implemented manually and acceptance criteria are complete.
- `/review-pr` stamps `related_pr` and submission status only when a PR submits the actual implementation or workflow deliverable. PRs that merely land brainstorms, plans, strategy docs, or other source artifacts should use `artifact_pr` without advancing source ideas past `planned`.
- Source ideas that produce durable docs or strategy with no implementation expected may use `status: resolved` by convention, with `related_doc` or `related_strategy` pointing at the output.
- `/wiki-sync` may archive the source idea only after merge, and only when the whole linked idea/plan chain is complete.
